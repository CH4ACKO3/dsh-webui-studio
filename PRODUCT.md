...
Success means a developer can start from a new package or an existing repository, test the Draft against a representative DSH profile, trace previewed UI back to relevant source, make a plugin-shaped change, and verify the activated result while the stable DSH environment remains usable.
...
Studio is served by a stable local `dsh web` Host at `/dsh-harmony/studio`. The stable Host owns the Studio interface, Draft registry, and Agent sessions. Each running Draft is previewed by a separate child `dsh web` Host inside Studio's main iframe, which can expand to fullscreen.

Developers may create a minimal new Web Client plugin or create a Draft from an existing local Git repository or Git URL. A package path selects a package within a monorepo. Draft repositories, worktrees, runtime homes, and registry records live under the stable Harness home's `studio` directory.
...
The product name is `dsh-webui-studio`, displayed as WebUI Studio inside DSH. Its voice is native, precise, and calm: terminology should follow DSH and Harmony concepts instead of introducing a separate low-code vocabulary.
