...
const messages = {
  ...
  saveElementToSource: { en: 'Save defaults to source', 'zh-CN': '保存默认值到源码' },
  savingElementToSource: { en: 'Saving defaults…', 'zh-CN': '正在保存默认值…' },
  elementSourceSaved: { en: 'Saved defaults in {count} source file(s).', 'zh-CN': '已将默认值保存到 {count} 个源码文件。' },
  elementSourceSaveNote: { en: 'Only variables with default-source anchors can be saved. The other controls remain Preview-only.', 'zh-CN': '只有声明默认值源码锚点的变量可以保存；其它控件仅作用于 Preview。' },
  ...
  variableNote: { en: 'Controls change the live Preview. Use Save default to source for variables with a default-source anchor.', 'zh-CN': '控件会修改实时 Preview；带有默认值源码锚点的变量可以使用“保存默认值到源码”。' },
